...
CREATE PROCEDURE [Col].[tf_AvgPortfolioOverdue] (@DateFrom date, @DateTo date, @CollectorId uniqueidentifier = null) as 
-- Эта процедура возвращает только просроченную часть
...
(
    select top 1
        cb.OverdueAmount
        + cb.OverdueRestructAmount
        + cb.OverduePercent
        + cb.OverdueRestructPercent
        + cb.Commission
        + cb.Fine as TotalDebt
