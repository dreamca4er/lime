...
select
    ...
    , reg.id as RegulatorId
from rr.Responses resp, rr.Regulators reg
where resp.CanBeChosen = 1
;
...
(
    values
    ('000000', N'{{Indent}}В ответ на Ваш запрос № {{IncomingNumber}} от {{IncomingDate}} г. (далее - Запрос), поступивший в адрес {{CompanyName}} (далее - Общество) в связи с рассмотрением обращения {{Placeholder}} (далее - Заявитель) {{Placeholder}}, предоставляем следующие сведения и документы.', 0, 0, 100)
    , ('000010', N'<b>{{Indent}}По пункту {{Placeholder}} Запроса.</b>', 0, 50, 100)
    , ('010100', N'{{Indent}}Между Обществом и Заявителем отсутствуют договорные отношения, поэтому предоставить информацию по существу пунктов Запроса не представляется возможным.', 0, 100, 100)
    , ('020100', N'{{Indent}}Между Заемщиком ({{ClientName}} {{ClientBirthday}} г.р.) и Обществом был(и) заключен(ы) следующий(е) Договор(ы) потребительского кредита (займа):', 0, 200, 100)
    , ('020200', N'№ {{ContractNumber}} от {{ContractDate}} г., обязательства по которому {{ContractState}}.', 1, 300, 100)
    , ('020300', N'№ {{ActiveContractNumber}} от {{ActiveContractDate}} (далее по тексту – Договор, Договоры), обязательства по которому {{ActiveContractState}}.', 1, 400, 100)
    , ('030100', N'{{Indent}}Вышеуказанный(е) Договор(ы) состоит(ят) из Общих условий договоров микрозайма, действующих на дату заключения Договора(ов), и Индивидуальных условий договора потребительского кредита (займа).

{{Indent}}Копия(и) Общих условий, действующих на дату заключения Договора(ов), и Индивидуальных условий приводятся в приложении к настоящему Ответу на Запрос.', 0, 500, 100)
    , ('040100', N'{{Indent}}При заключении Договора(ов) займа {{ContractsList}} Заемщиком были предоставлены:

{{Indent}}• Согласие Заемщика (должника) передавать (сообщать) третьим лицам или делать доступными для них сведения о должнике, просроченной задолженности и ее взыскании и любые другие персональные данные Заемщика (должника);

{{Indent}}• Согласие Заемщика (должника) на осуществление направленного на возврат его просроченной задолженности взаимодействия с третьим лицом.', 0, 600, 100)
    , ('040200', N'{{Indent}}При заключении Договора(ов) займа {{ContractsList}} Заемщиком были предоставлены:

{{Indent}}• Согласие Заемщика (должника) передавать (сообщать) третьим лицам или делать доступными для них сведения о должнике, просроченной задолженности и ее взыскании и любые другие персональные данные Заемщика (должника);

{{Indent}}• Согласие Заемщика (должника) на осуществление направленного на возврат его просроченной задолженности взаимодействия с третьим лицом;

{{Indent}}• Согласие на получение кредитного отчета.', 0, 700, 100)
    , ('040300', N'{{Indent}}При заключении Договора(ов) займа {{ContractsList}} Заемщиком были предоставлены:

{{Indent}}• Согласие на обработку персональных данных;

{{Indent}}• Согласие на получение кредитного отчета.', 0, 800, 100)
    , ('040400', N'{{Indent}}Все вышеуказанные согласия приводятся в приложении.', 0, 900, 100)
    , ('050100', N'{{Indent}}Продление Договора(ов) осуществлялось в соответствии с правилами, установленными Общими условиями и Гражданским кодексом Российской Федерации (далее – ГК РФ)
{{ProlongationsList}}
{{Indent}}В силу п. 9.2 (ранее 10.8, ранее в соответствии с нормами ГК РФ) Общих условий получение услуги Продление срока возврата Займа осуществляется полностью по усмотрению Заемщика и не иначе, как по добровольной инициативе Заемщика при условии погашения всех обязательств Заемщика по Договору, за исключением возврата основного долга и Платы за выбранный Заемщиком канал выдачи (при наличии).
{{Indent}}В соответствии с п. 9.3 (ранее 10.9, ранее в соответствии с нормами ГК РФ) Общих условий Продление срока возврата Займа осуществляется путем заключения соответствующего соглашения об изменении Индивидуальных условий в письменной форме в порядке, предусмотренном пунктом 3 статьи 438 ГК РФ.
{{Indent}}В силу п. 3 ст. 438 ГК РФ совершение лицом, получившим оферту, в срок, установленный для ее акцепта, действий по выполнению указанных в ней условий договора, считается акцептом, если иное не предусмотрено законом, иными правовыми актами или не указано в оферте.
{{Indent}}Согласно п. 2 ст. 434 ГК РФ договор в письменной форме может быть заключен путем составления одного документа, подписанного сторонами, а также путем обмена письмами, телеграммами, телексами, телефаксами и иными документами, в том числе электронными документами, передаваемыми по каналам связи, позволяющими достоверно установить, что документ исходит от стороны по договору.
{{Indent}}Электронным документом, передаваемым по каналам связи, признается информация, подготовленная, отправленная, полученная или хранимая с помощью электронных, магнитных, оптических либо аналогичных средств, включая обмен информацией в электронной форме и электронную почту.
{{Indent}}Согласно п. 3 ст. 434 ГК РФ письменная форма договора считается соблюденной, если письменное предложение заключить договор принято в порядке, предусмотренном п. 3 ст. 438 ГК РФ.
{{Indent}}С учетом вышеизложенного, сообщаем сведения о том, что продление срока возврата займа по указанным договорам осуществлялось путем увеличения срока возврата займа по правилам, установленным п. 2 и 3 ст. 434, п. 3 ст. 438 ГК РФ, путем обмена электронными сообщениями и совершением конклюдентных действий по выполнению условий, указанных в Общих условиях.
{{Indent}}В связи с тем, что дополнительные соглашения в виде единого документа, подписанного сторонами, о продлении срока возврата займа по указанным договорам не оформлялись, Обществом указанные дополнительные соглашения предоставлены быть не могут.
{{Indent}}Сведения о направлении в адрес Заемщика SMS-сообщений и E-mail-сообщений, а также сведения о внесении Заемщиком денежных средств в погашение процентов, подтверждающих совершение акцепта оферты (выполнение условий, указанных в Общих условиях) в целях продления срока займа по указанным договорам, приводятся в приложении к настоящему Ответу на Запрос.', 0, 1000, 100)
    , ('050201', N'{{Indent}}Также между Заемщиком и Обществом было(и) заключено(ы) следующее(ие) соглашение(я) об изменении индивидуальных условий займа (далее – Соглашение(я)), копия(и) которого(ых) приводится(ятся) в приложении к настоящему Ответу на Запрос.', 0, 1100, 100)
    , ('050202', N'{{Indent}}К договору № {{ContractNumber}} от {{ContractDate}} г.:
{{AgreementList}}', 0, 1200, 100)
    , ('050203', N'{{Indent}}Иных приложений, дополнений, изменений и иных неотъемлемых частей к Договорам займа нет, поэтому предоставлены быть не могут.', 0, 1300, 100)
    , ('050300', N'{{Indent}}Дополнительные соглашения (в том числе индивидуальные условия Договора(ов) с внесенными в них на основании дополнительных соглашений изменениями) об изменении условий Договора(ов), его(их) сроков, сумм плановых платежей, предусмотренных графиком(ами) платежей по Договору(ам) (графиков платежей), между Заемщиком и Обществом не заключались, в связи с чем предоставить копии данных документов по состоянию на каждую из дат внесения изменений, с приложением подтверждающих документов, свидетельствующих об уведомлении Заявителя о внесении изменений в Договор(ы) не представляется возможным.', 0, 1400, 100)
    , ('050400', N'{{Indent}}Заемщик предоставил заявление(я) о передаче третьему лицу суммы по {{InsurancesList}} для оплаты страхового полиса. Копия(и) заявления(й) о передаче средств, страховой(ые) полис(ы) и особые условия страхования приводятся в приложении.', 0, 1500, 100)
    , ('050500', N'{{Indent}}Иные приложения, дополнения и неотъемлемые части к Договору(ам) отсутствуют, поэтому предоставлены быть не могут.', 0, 1600, 100)
    , ('060000', N'{{Indent}}Копия(и) правил предоставления займов (микрозаймов), утвержденных Обществом в установленном порядке, на основании которых был(и) заключен(ы) Договор(ы), приводится(ятся) в приложении к настоящему Ответу на Запрос.', 0, 1700, 100)
    , ('070100', N'{{CommonConditionsName}}', 1, 1800, 100)
    , ('070200', N'Индивидуальные условия договора № {{ContractNumber}} от {{ContractDate}} г.;', 1, 1900, 100)
    , ('070300', N'Индивидуальные условия договора № {{ContractNumber}} от {{ContractDate}} г. с Согласиями;', 1, 1900, 100)
    , ('070400', N'Соглашение от {{ConditionsChangeAgreementDate}} об изменении индивидуальных условий займа {{ContractNumber}} от {{ContractDate}} г.;', 1, 1900, 200)
    , ('070500', N'{{RulesName}}', 1, 2000, 100)
    , ('070600', N'Документы по договору страхования № {{InsuranceContract}} от {{InsuranceDate}} г.;', 1, 2100, 100)
