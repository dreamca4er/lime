if 1=0
    ...
    (
        Alias,FullName,Address,RegulatorId,CreatedBy
    )
    select *
        , 1 as RegulatorId
        , 0x44 as CreatedBy
    ...
    --drop table if exists rr.ResponceTemplates
    create table rr.ResponceTemplates
    (
        id int identity(1, 1)
        , TemplateId int not null
        , TemplatePart nvarchar(10) not null
        , TemplateBody nvarchar(max) not null
        , AddLineNumber bit not null default(0)
        , Timestamp timestamp
        , constraint PK_rr_ResponceTemplates
            primary key (TemplateId, TemplatePart)
    )
    ...
    @level0type = N'SCHEMA', @level0name = 'rr',  
    @level1type = N'TABLE',  @level1name = 'ResponceTemplates'
    ;
    
    insert into rr.ResponceTemplates
    (
        TemplateId,TemplatePart,TemplateBody,AddLineNumber
    )
    select *
    from 
    (
        values
        (1, '010100', N'Между Обществом и Заявителем отсутствуют договорные отношения, поэтому предоставить информацию по существу пунктов Запроса не представляется возможным.', 0)
        , (2, '020100', N'Между Заемщиком ({ClientName} {ClientBirthday} г.р.) и Обществом был(и) заключен(ы) следующий(е) Договор(ы) потребительского кредита (займа):', 0)
        , (2, '020200', N'№ {ContractNumber} от {ContractDate}, обязательства по которому Заемщиком {ContractState}.', 1)
        , (2, '020300', N'№ {ActiveContractNumber} от {ActiveContractDate} (далее по тексту – Договор, Договоры), обязательства по которому Заемщиком {ActiveContractState}.', 1)
        , (3, '030100', N'Вышеуказанный(е) Договор(ы) состоит(ят) из Общих условий договоров микрозайма, действующих на дату заключения Договора(ов), и Индивидуальных условий договора потребительского кредита (займа).
    
    Копия(и) Общих условий, действующих на дату заключения Договора(ов), и Индивидуальных условий приводятся в приложении к настоящему Ответу на Запрос.', 0)
        , (4, '040100', N'При заключении Договора(ов) займа № {ContractsList} Заемщиком были предоставлены:
    ...
    • Согласие Заемщика (должника) на осуществление направленного на возврат его просроченной задолженности взаимодействия с третьим лицом.', 0)
        , (4, '040200', N'При заключении Договора(ов) займа № {ContractsList} Заемщиком были предоставлены:
    ...
    • Согласие на получение кредитного отчета.', 0)
        , (4, '040300', N'При заключении Договора(ов) займа № {ContractsList} Заемщиком были предоставлены:
    
    • Согласие на обработку персональных данных;
    
    • Согласие на получение кредитного отчета.', 0)
        , (4, '040400', N'Все вышеуказанные согласия приводятся в приложении.', 0)
        , (5, '050100', N'Продление Договора(ов) осуществлялось в соответствии с правилами, установленными Общими условиями и Гражданским кодексом Российской Федерации (далее – ГК РФ)
    {ProlongationsList}
    ...
    Сведения о направлении в адрес Заемщика SMS-сообщений и E-mail-сообщений, а также сведения о внесении Заемщиком денежных средств в погашение процентов, подтверждающих совершение акцепта оферты (выполнение условий, указанных в Общих условиях) в целях продления срока займа по указанным договорам, приводятся в приложении к настоящему Ответу на Запрос.', 0)
        , (5, '050201', N'Также между Заемщиком и Обществом было(и) заключено(ы) следующее(ие) соглашение(я) об изменении индивидуальных условий займа (далее – Соглашение(я)), копия(и) которого(ых) приводится(ятся) в приложении к настоящему Ответу на Запрос.', 0)
        , (5, '050202', N'К договору № {ContractNumber} от {ContractDate}:', 0)
        , (5, '050203', N'Соглашение от {AgreementDate};', 1)
        , (5, '050204', N'Иных приложений, дополнений, изменений и иных неотъемлемых частей к Договорам займа нет, поэтому предоставлены быть не могут.', 0)
        , (5, '050300', N'Дополнительные соглашения (в том числе индивидуальные условия Договора(ов) с внесенными в них на основании дополнительных соглашений изменениями) об изменении условий Договора(ов), его(их) сроков, сумм плановых платежей, предусмотренных графиком(ами) платежей по Договору(ам) (графиков платежей), между Заемщиком и Обществом не заключались, в связи с чем предоставить копии данных документов по состоянию на каждую из дат внесения изменений, с приложением подтверждающих документов, свидетельствующих об уведомлении Заявителя о внесении изменений в Договор(ы) не представляется возможным.', 0)
        , (5, '050400', N'Заемщик предоставил заявление(я) о передаче третьему лицу суммы по № {InsurancesList} для оплаты страхового полиса. Копия(и) заявления(й) о передаче средств, страховой(ые) полис(ы) и особые условия страхования приводятся в приложении.', 0)
        , (5, '050500', N'Иные приложения, дополнения и неотъемлемые части к Договору(ам) отсутствуют, поэтому предоставлены быть не могут.', 0)
        , (6, '060000', N'Копия(и) правил предоставления займов (микрозаймов), утвержденных Обществом в установленном порядке, на основании которых был(и) заключен(ы) Договор(ы), приводится(ятся) в приложении к настоящему Ответу на Запрос.', 0)
    ) t(TemplateId, TemplatePart, TemplateBody, AddLineNumber)
    ;
    ...
    (
        ...
        , ('01.07.2019', N'Общие условия договоров микрозаймов МФК "Лайм-Займ" (ООО) от 26.06.2019')
        , ('02.07.2019', N'Общие условия договоров микрозаймов МФК "Лайм-Займ" (ООО) от 26.06.2019')
        , ('04.09.2019', N'Общие условия договоров микрозаймов МФК "Лайм-Займ" (ООО) от 26.06.2019')
        , ('12.09.2019', N'Общие условия договоров микрозаймов МФК "Лайм-Займ" (ООО) от 26.06.2019')
        , ('08.10.2019', N'Общие условия договоров микрозаймов МФК "Лайм-Займ" (ООО) от 26.06.2019')
    ...
    (
        ...
        , ('01.07.2019', N'Общие условия договора микрозайма ООО МФК "МангоФинанс" от 26.06.2019')
        , ('02.07.2019', N'Общие условия договора микрозайма ООО МФК "МангоФинанс" от 26.06.2019')
        , ('04.09.2019', N'Общие условия договора микрозайма ООО МФК "МангоФинанс" от 26.06.2019')
        , ('12.09.2019', N'Общие условия договора микрозайма ООО МФК "МангоФинанс" от 26.06.2019')
        , ('08.10.2019', N'Общие условия договора микрозайма ООО МФК "МангоФинанс" от 26.06.2019')
    ) v(DateFrom, ConditionsName)
    ...
    (
        ...
        , DateFrom datetime2 not null primary key
        
    )
    ;
    
    insert rr.ResponseSignatures
    (
        FileName,DateFrom
    )
    select 
        N'ЭЦП_Манго_20181029' as FileName
        , '20181029' as DateFrom
    ...
    select 
        N'ЭЦП_Лайм_20190109' as FileName
        , '20190109' as DateFrom
    from bi.ProjectConfig 
    where ProjectId = 0
    ;
    ...
    EXEC sys.sp_addextendedproperty   
    @name = N'MS_Description',   
    @value = N'Список подписей ответа с датами старта BR-13829',   
    @level0type = N'SCHEMA', @level0name = 'rr',  
