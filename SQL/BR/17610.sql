...
update c set Schedule = s.Schedule, Psk = s.Psk
from #credits c
inner join "OLD-PROJECT-DB".LimeZaim_Website.dbo.AdditionalCreditInfo s on s.CreditId = c.ProductId
;
...
inner join #credits c on c.ProductId = cb.CreditId
    and cb.Date < @NewProjectStartedOn
;
...
(
    ...
    where cb.InfoType = 'payment'
        and cb.DateOperation >= @NewProjectStartedOn
) cp
;
