...
select
    ...
    , l.Date
    , l.Type
    , cast(p8.Address as nvarchar(400)) as Address
into dbo.br6778_parsed
from dbo.br6778_lime_ready l
outer apply
(
...
create index IX_dbo_br6778_parsed on dbo.br6778_parsed(ProjectClientId, Type, Date)
/
--select
--    l.*
--    , ccl.RegAddressStr
...
select distinct ccl.RegAddressStr
from dbo.br6778_2 l
inner join client.ClientCardLog ccl on ccl.ClientId = l.ClientId
where len(ccl.RegAddressStr) > 90
/

select * 
from (select N'182113 РОССИЯ ПСКОВСКАЯ ОБЛ ВЕЛИКИЕ ЛУКИ Г К.ЛИБКНЕХТА УЛ 9/14 Д 18 КВ, 182113 РОССИЯ ПСКОВСКАЯ ОБЛ ВЕЛИКИЕ ЛУКИ Г К.ЛИБКНЕХТА УЛ 9/14 Д 18 КВ' a) b
